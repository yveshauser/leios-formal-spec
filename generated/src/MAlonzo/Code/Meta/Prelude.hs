{-# LANGUAGE BangPatterns #-}
{-# LANGUAGE EmptyCase #-}
{-# LANGUAGE EmptyDataDecls #-}
{-# LANGUAGE ExistentialQuantification #-}
{-# LANGUAGE NoMonomorphismRestriction #-}
{-# LANGUAGE OverloadedStrings #-}
{-# LANGUAGE PatternSynonyms #-}
{-# LANGUAGE RankNTypes #-}
{-# LANGUAGE ScopedTypeVariables #-}

{-# OPTIONS_GHC -Wno-overlapping-patterns #-}

module MAlonzo.Code.Meta.Prelude where

import MAlonzo.RTE (coe, erased, AgdaAny, addInt, subInt, mulInt,
                    quotInt, remInt, geqInt, ltInt, eqInt, add64, sub64, mul64, quot64,
                    rem64, lt64, eq64, word64FromNat, word64ToNat)
import qualified MAlonzo.RTE
import qualified Data.Text
import qualified MAlonzo.Code.Agda.Builtin.Maybe
import qualified MAlonzo.Code.Agda.Builtin.Sigma
import qualified MAlonzo.Code.Data.Bool.Base
import qualified MAlonzo.Code.Data.List.Base

-- Meta.Prelude.lookupᵇ
d_lookup'7495'_8 ::
  () ->
  () ->
  (AgdaAny -> AgdaAny -> Bool) ->
  [MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14] ->
  AgdaAny -> Maybe AgdaAny
d_lookup'7495'_8 ~v0 ~v1 v2 v3 v4 = du_lookup'7495'_8 v2 v3 v4
du_lookup'7495'_8 ::
  (AgdaAny -> AgdaAny -> Bool) ->
  [MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14] ->
  AgdaAny -> Maybe AgdaAny
du_lookup'7495'_8 v0 v1 v2
  = case coe v1 of
      [] -> coe MAlonzo.Code.Agda.Builtin.Maybe.C_nothing_18
      (:) v3 v4
        -> case coe v3 of
             MAlonzo.Code.Agda.Builtin.Sigma.C__'44'__32 v5 v6
               -> coe
                    MAlonzo.Code.Data.Bool.Base.du_if_then_else__44 (coe v0 v5 v2)
                    (coe MAlonzo.Code.Agda.Builtin.Maybe.C_just_16 (coe v6))
                    (coe du_lookup'7495'_8 (coe v0) (coe v4) (coe v2))
             _ -> MAlonzo.RTE.mazUnreachableError
      _ -> MAlonzo.RTE.mazUnreachableError
-- Meta.Prelude.zipWithIndex
d_zipWithIndex_28 ::
  () ->
  () -> (Integer -> AgdaAny -> AgdaAny) -> [AgdaAny] -> [AgdaAny]
d_zipWithIndex_28 ~v0 ~v1 v2 v3 = du_zipWithIndex_28 v2 v3
du_zipWithIndex_28 ::
  (Integer -> AgdaAny -> AgdaAny) -> [AgdaAny] -> [AgdaAny]
du_zipWithIndex_28 v0 v1
  = coe
      MAlonzo.Code.Data.List.Base.du_zipWith_104 (coe v0)
      (coe
         MAlonzo.Code.Data.List.Base.d_upTo_418
         (coe MAlonzo.Code.Data.List.Base.du_length_284 v1))
      (coe v1)
-- Meta.Prelude.enumerate
d_enumerate_36 ::
  () -> [AgdaAny] -> [MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14]
d_enumerate_36 ~v0 = du_enumerate_36
du_enumerate_36 ::
  [AgdaAny] -> [MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14]
du_enumerate_36
  = coe
      du_zipWithIndex_28
      (coe MAlonzo.Code.Agda.Builtin.Sigma.C__'44'__32)
